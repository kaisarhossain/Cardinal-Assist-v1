# cardinal_assist_app_v2.py - Dynamic FAQ version
# Streamlit App
...
# ============================================================================
# DYNAMIC FAQ LOADING FROM SINGLE JSON FILE
# ============================================================================

def load_faq_categories_from_json(json_file="faq_categories.json"):
    # Check if JSON file exists
    if not os.path.exists(json_file):
        st.warning(f"FAQ file '{json_file}' not found. Creating default file...")
        create_default_faq_json(json_file)
    ...
    try:
        ...
        # Validate structure
        if not isinstance(faq_categories, dict):
            st.error(f"Invalid JSON format in {json_file}. Expected a dictionary.")
            return {}

        # Filter out empty categories
        faq_categories = {k: v for k, v in faq_categories.items() if v and isinstance(v, list)}
        # st.success(f"✅ Loaded {len(faq_categories)} categories from {json_file}")
        # print(f"✅ Loaded {len(faq_categories)} categories from {json_file}")
        return faq_categories

    except json.JSONDecodeError as e:
        st.error(f"Error parsing JSON file {json_file}: {e}")
        return {}
...
def create_default_faq_json(json_file):
    """Create default FAQ JSON file"""
    default_categories = {
        "🎓 Admissions": [
            "What are the admission requirements for undergraduate students?",
            "What are the admission requirements for graduate students?",
    ...
    try:
        ...
        st.success(f"✅ Created default FAQ file: {json_file}")
    except Exception as e:
        st.error(f"Error creating {json_file}: {e}")
...
def generate_conversation_text():
    """Generate formatted text of the conversation for download"""
    if not st.session_state.chat_history:
        return "No conversation to save yet."

    conversation_text = "=" * 80 + "\n"
    conversation_text += "CARDINAL ASSIST - CONVERSATION HISTORY\n"
    conversation_text += "=" * 80 + "\n\n"
    conversation_text += f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
    conversation_text += f"Total Questions: {len(st.session_state.chat_history)}\n"
    conversation_text += "=" * 80 + "\n\n"

    for i, chat in enumerate(st.session_state.chat_history, 1):
        conversation_text += f"[Q{i}] {chat['timestamp']}\n"
        conversation_text += f"YOU: {chat['question']}\n\n"
        conversation_text += f"CARDINAL ASSIST: {chat['answer']}\n"

        # Add sources if available
        if chat.get('chunks'):
            conversation_text += f"\nSOURCES:\n"
            for j, chunk in enumerate(chat['chunks'][:3], 1):
                conversation_text += f"  {j}. {chunk['source']} (relevance: {chunk['score']:.2f})\n"

        conversation_text += "\n" + "-" * 80 + "\n\n"

    conversation_text += "=" * 80 + "\n"
    conversation_text += "End of Conversation\n"
    conversation_text += "=" * 80 + "\n"
...
def main():
    ...
    # ✅ Load FAQ categories dynamically from JSON file
    FAQ_CATEGORIES = load_faq_categories_from_json("faq_categories.json")

    # If no categories loaded, show error and stop
    if not FAQ_CATEGORIES:
        st.error("No FAQ categories available. Please check faq_categories.json file.")
        st.stop()
    ...
    with st.sidebar:
        ...
        # ✅ FAQ Categories (dynamically loaded)
        ...
        with col_save:
            if st.session_state.chat_history:
                ...
                filename = f"cardinal_assist_{timestamp}.txt"
                ...
                st.button("💾 Save", use_container_width=True, type="secondary", disabled=True)
